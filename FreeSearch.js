...
;(function($){
    ...
    function inputBoxInit($frame){
        ...
        $inputEle.resize = function(){
            ...
            // 点击样式
            $inputEle.css({
                width:isTooShort ? $frame.width() : $frame.width() - lastSpanPos.left,
                paddingLeft:isTooShort ? 5 :lastSpanPos.left,
                paddingTop:isTooShort ? (lastSpanPos.top + $lastSpan.outerHeight()) : lastSpanPos.top,
                overflow:"hidden",
                height:"auto",
                border:"none",
                float:"left"
    ...
    function itemsBoxInit($frame){
        ...
        $itemsBox.createItem = function(name,id,email){
            ...
            var width = Math.max($frame.width(),($item.position().left + $item.outerWidth()));
            $frame.css({
                height:newHeight,
                width:width
            });
